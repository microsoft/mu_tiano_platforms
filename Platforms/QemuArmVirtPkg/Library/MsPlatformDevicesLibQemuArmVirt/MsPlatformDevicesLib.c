...
STATIC PREFERRED_VIDEO_DEVICE  gPreferredVideo = {
  gPciRootBridge,
  {
    ...
    0x00,  // Function
    0x1F   // Device  -- pinned to match -device bochs-display,addr=0x1f
  },
  gEndEntire
};
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      //
      // Accept any PCI display controller (class 0x03/*) -- including bochs
      // (class 0x03/0x80 "Display Other") and virtio-gpu -- not just legacy
      // VGA. Matches the broader IS_PCI_DISPLAY check used elsewhere in this
      // file (e.g. DetectAndPreparePlatformPciDevicePath).
      //
      if (IS_PCI_DISPLAY (&Pci) || IS_PCI_OLD_VGA (&Pci)) {
        DEBUG ((DEBUG_INFO, "  \nPCI Display Device Found\n"));
        return TRUE;
      }
    }
  }
