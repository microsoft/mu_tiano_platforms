...
/** MU_CHANGE START
  Constructor for TEST_VERSION_RECORD_CERT that creates the following VERSION_RECORD_CERT:
    [
  ...
  @param[in] Cert  The TEST_VERSION_RECORD_CERT to populate.
**/
STATIC
VOID
TestVersionRecordCert (
  OUT TEST_VERSION_RECORD_CERT* Cert
  )
{
  Cert->header.RecordSize = sizeof (TEST_VERSION_RECORD_CERT);
  Cert->header.StrucVersion = VERSION_RECORD_RULE_VERSION;
  Cert->header.u1.s1.Rule1 = VERSION_RECORD_RULE_WILDCARD;
  Cert->header.u1.s1.Rule2 = VERSION_RECORD_RULE_WILDCARD;
  Cert->header.u1.s1.Rule3 = VERSION_RECORD_RULE_LESS_THAN;
  Cert->header.u1.s1.Rule4 = VERSION_RECORD_RULE_WILDCARD;
  Cert->header.CompanyNameSize = sizeof (TEST_RULE_COMPANY_NAME_STR);
  Cert->header.OriginalFilenameSize = sizeof (TEST_RULE_ORIGINAL_FILENAME_STR);
  Cert->header.FileVersion[0] = 10;
  Cert->header.FileVersion[1] = 0;
  Cert->header.FileVersion[2] = 18362;
  Cert->header.FileVersion[3] = 1;
  StrCpyS (Cert->CompanyName, TEST_RULE_COMPANY_NAME_LEN, TEST_RULE_COMPANY_NAME_STR);
  StrCpyS (Cert->OriginalFilename, TEST_RULE_ORIGINAL_FILENAME_LEN, TEST_RULE_ORIGINAL_FILENAME_STR);
}
// MU_CHANGE END
  ...
  )
{
  ...
  UINTN      SizeOfPkKek1;
  BOOLEAN    NoDefault;
  TEST_VERSION_RECORD_CERT TestCert; // MU_CHANGE
  ...
  // warnings.
  //
  PkKek1 = dummy_cert; // MU_CHANGE add dummy cert
  SizeOfPkKek1 = sizeof (dummy_cert); // MU_CHANGE add dummy cert
  ...
  // MU_CHANGE START
  // Enroll test Version Record Cert in dbx
  //
  TestVersionRecordCert (&TestCert); // MU_CHANGE
  Status = EnrollListOfCerts (
             EFI_IMAGE_SECURITY_DATABASE1,
             &gEfiImageSecurityDatabaseGuid,
             &gEfiCertVersionRecordResourceGuid,
             &TestCert, sizeof (TestCert), &gTestCertGuid,
             NULL);
  // MU_CHANGE END
  if (EFI_ERROR (Status)) {
    goto FreePkKek1;
