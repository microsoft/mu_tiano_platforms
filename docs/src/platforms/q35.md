...
By solely focusing on the Q35 chipset, this package can be optimized such that it is allowed to break compatibility with
other QEMU supported chipsets. The Q35 chipset can be paired with an IA32 or X64 processor to enable a machine that can
emulate PC class hardware with industry standard features like SMM and PCI-E.
...
Q35 is a machine type that QEMU emulates. Below is a diagram from Qemu.org about the Q35 chipset which emulates a ICH9
(I/O controller hub).
...
QemuQ35Pkg uses the Project Mu repositories and Edk2 PyTools for its build operations. Specific details can be found
...
Current QEMU Q35 platform supports the following features provided by Project Mu:
...
[Details](../features/q35/feature_codeql.md)

### Color Bar

Color bars are used to quickly convey the Device state, based upon the DeviceStateLib. Color bars are displayed by the
ColorBarDisplayDeviceStateLib.

[Details](../features/q35/feature_colorbar.md)
...
[Details](../features/q35/feature_config.md)
...
code module that executes within OS context. This feature adds the PRM infrastructure to the firmware that enables
loading PRM modules which in turn are exposed to the OS for invocation. To accomplish this, a set of open source sample
PRM modules are used to demonstrate the feature and show how additional modules can be added.

[Details](../features/q35/feature_prm.md)
...
[Details](../features/q35/feature_tpm_replay.md)

### UEFI Memory Protections

UEFI Memory Protections add safety functionality such as page and pool guards, stack guard, and null pointer detection.
The settings are split between MM and DXE environments for modularity.

[Details](../features/common/feature_memoryprotection.md)
...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MsPlatformDevicesLib**   | [MsPlatformDevicesLib](https://github.com/microsoft/mu_tiano_platforms/blob/main/Platforms/QemuQ35Pkg/Library/MsPlatformDevicesLibQemuQ35/ReadMe.md) |
| **PlatformDebugLibIoPort** | [PlatformDebugLibIoPort](https://github.com/microsoft/mu_tiano_platforms/blob/main/Platforms/QemuQ35Pkg/Library/PlatformDebugLibIoPort/ReadMe.md)    |
| **PlatformThemeLib**       | [PlatformThemeLib](https://github.com/microsoft/mu_tiano_platforms/blob/main/QemuPkg/Library/PlatformThemeLib/ReadMe.md)                             |
